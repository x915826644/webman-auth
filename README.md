...
```
composer require Jody/webman-auth
```
# 配置文件
```
//路径 config/plugin/Jody/auth/app.php
...
```
php webman Jody:auth

```
2. 加密密码

```php
use Jody\WebmanAuth\Facade\Auth;
...
```
    ...
    //config/plugin/Jody/auth/app.php设置
    'guard' => [
         'user' => [ // 普通用户
             'key' => 'id', //主键
             'field' => ['id','username','email','mobile','avatar'], //设置允许写入扩展中的字段 一般为数据表存在的字段
